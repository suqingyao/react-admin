...
import { getDarkColor, getLightColor, setElementThemeColor } from '@/lib/ui';
...
export function useTheme() {
  ...
  const setSystemAutoTheme = () => {
    const theme = getPrefersDark() ? SystemThemeEnum.DARK : SystemThemeEnum.LIGHT;
...
export function initializeTheme() {
  ...
  const applyThemeByMode = () => {
    ...
    if (settingStore.systemThemeMode === SystemThemeEnum.AUTO) {
      actualTheme = getPrefersDark() ? SystemThemeEnum.DARK : SystemThemeEnum.LIGHT;
      // 更新实际应用的主题类型
  ...
  // 如果是 AUTO 模式，监听系统主题变化（使用原生 matchMedia）
  if (settingStore.systemThemeMode === SystemThemeEnum.AUTO) {
    if (typeof window === 'undefined' || typeof window.matchMedia !== 'function') {
      return;
    }
