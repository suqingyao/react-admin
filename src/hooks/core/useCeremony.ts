...
import { useIntervalFn, useTimeoutFn } from '@/hooks/shared';
import eventBus from '@/lib/sys/event-emitter';
...
export function useCeremony() {
  ...
  const { holidayFireworksLoaded, isShowFireworks } = settingStore;

  const playedCountRef = useRef(0);
  const countRef = useRef<number>(FESTIVAL_CONFIG.DEFAULT_FIREWORKS_COUNT);
   ...
   * 获取当前日期对应的节日数据
   */
  const currentFestivalData = useMemo<FestivalConfig | undefined>(() => {
    const currentDate = dayjs().format('YYYY-MM-DD');
    return festivalConfigList.find((item) => isDateInRange(currentDate, item.date, item.endDate));
  }, [isDateInRange]);

  useEffect(() => {
    countRef.current = currentFestivalData?.count ?? FESTIVAL_CONFIG.DEFAULT_FIREWORKS_COUNT;
  }, [currentFestivalData]);
  ...
  const updateFestivalDate = () => {
    settingStore.setFestivalDate(currentFestivalData?.date || '');
  };
  ...
  const triggerFirework = () => {
    eventBus.emit('triggerFireworks', currentFestivalData?.image);
  };

  /**
   * 完成烟花效果后显示文本
   */
  const { start: startFestivalText } = useTimeoutFn(
    () => {
  ...
  const { pause, resume } = useIntervalFn(
    () => {
      ...
      if (playedCountRef.current >= countRef.current) {
        pause();
        showFestivalText();
      }
    },
  ...
  const { start: startInitialDelay } = useTimeoutFn(
    startFireworksLoop,
   ...
   */
  const openFestival = () => {
    if (!currentFestivalData || holidayFireworksLoaded) {
      return;
    }
